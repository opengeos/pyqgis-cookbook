...
From **Plugins --> Manage and Install plugins…**, install:
...
this can be done by editing `~/.bashrc` or `~/.bash-profile` with
...
For using [Pyscripter IDE ](https://github.com/pyscripter/pyscripter),
...
* [Aptana Studio 3 Plugin ](https://www.axway.com/en/aptana) or [PyDev ](https://www.pydev.org)
* QGIS 3.x
* You may also want to install **Remote Debug**, a QGIS plugin. At the moment
  it's still experimental so enable |checkbox| **Experimental plugins**
  under **Plugins --> Manage and Install plugins... --> Options**
  beforehand.
...
![Eclipse project](img/eclipsenewproject.png)
*Eclipse project*

#. Right-click your new project and choose **New --> Folder**.
...
Now in the view **Project Explorer**, your source tree pops up and you
...
#. Open the Console view (**Window --> Show view**).
   It will show the **Debug Server** console which is not very
   interesting. But there is a button **Open Console** which lets you
   change to a more interesting PyDev Debug Console.
#. Click the arrow next to the **Open Console** button and choose
...
![PyDev Debug Console](img/console-buttons.png)
*PyDev Debug Console*
...
**Tip:** A little bit annoying is, that every time you enter a command, the onsole switches back to the Debug Server. To stop this behavior, you can lick the *Pin Console* button when on the Debug Server page and it should emember this decision at least for the current debug session.
...
![PyDev Debug Console](img/interpreter-libraries.png)
*PyDev Debug Console*
...
#. You should also add your plugins folder here (it is in `python/plugins`
   under the *user profile <user_profiles>* folder).
...
**Note:** Every time the QGIS API changes (e.g. if you're compiling QGIS master and the SIP file changed), you should go back to this page and simply click :guilabel:`Apply`. This will let Eclipse parse all the libraries again.
...
build directory `~/dev/qgis/build/master`. It's not compulsory to have
a self compiled QGIS, but only this has been tested. Paths must be adapted.

#. In PyCharm, in your **Project Properties**, **Project Interpreter**,
...
#. Select a generic location for all your Python projects such as
   `~/dev/qgis/venv` because we will use this Python interpreter for all our plugins.
#. Choose a Python 3 base interpreter available on your system and check
   the next two options **Inherit global site-packages**
   and **Make available to all projects**.

![](img/pycharm-add-venv.png)
