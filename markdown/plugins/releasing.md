...
some people, do not hesitate to upload it to *official_pyqgis_repository*.
...
[OSGEO web portal ](https://www.osgeo.org/community/getting-started-osgeo/osgeo_userid/).
...
#. `metadata.txt` is required
#. all required metadata listed in *metadata table<plugin_metadata_table>*
   must be present
...
files are: `metadata.txt`,  `__init__.py` and `LICENSE`.
But it would be nice to have a `README` and of course an icon to represent
